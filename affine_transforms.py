...
def define_matrix(x):
    """
    Given a point  of coordinate(x, y, z), this function return the va
    """
    translate = np.array([[1, 0, 0, 0.5], [1, 0, 0, 0.5], [1, 0, 0, 0.5], [0, 0, 0, 1]])
    scale = np.array([[0.5, 0, 0, 0], [0, 0.5, 0, 0], [0, 0, 0.5, 0], [0, 0, 0, 1]])
    x_w = np.insert(x, [4], [1], axis=1)
    x_w.transpose()
    #np.delete(transpose, 4, 1)
    return x

def transform_matrix_offset_center(matrix, x):
...
#     new_data = np.array([[x], [y], [z], [1]])

    #for i int x:
        

    x_w = np.insert(x, 3, [1], axis=1)
#     print("x_w=", x_w)
#     x_w = x_w.transpose()
        
#     print("new data", x.shape)
    transform_matrix = np.dot(x_w, matrix)
    return transform_matrix

def apply_transform_old(x, transform, fill_mode='nearest', fill_value=0.):
    ...
    print("final_affine_matrix", final_affine_matrix)
...
def apply_transform(x, transform, fill_mode='nearest', fill_value=0.):
    ...
    """
    x = x.astype('float32')
    transform = transform_matrix_offset_center(transform, x)
    final_affine_matrix = transform[:3500, :3]
#     final_offset = transform[:3, 3]
#     channel_images = [ndi.interpolation.affine_transform(x_channel, final_affine_matrix,
#             final_offset, order=0, mode=fill_mode, cval=fill_value) for x_channel in x]
#     x = np.stack(channel_images, axis=0)
...
class AffineCompose(object):
    ...
    def __call__(self, x, y=None):
        ...
        tform_matrix = self.transforms[0](x)
        for tform in self.transforms[1:]:
            print("tform_matrix=", tform_matrix)
            print("tform=", tform)
            tform_matrix = np.dot(tform_matrix, tform(x)) 
            print("tform_matrix=", tform_matrix)
...
class Translation(object):
    ...
    def __init__(self, 
        ...
        self.depth_range = translation_range[1]
        self.fill_mode = fill_mode
        self.fill_value = fill_value
        self.target_fill_mode = target_fill_mode
        self.target_fill_value = target_fill_value
    ...
    def __call__(self, x, y=None):
        ...
        if self.height_range > 0:
            tx = random.uniform(-self.height_range, self.height_range) * x[0][0]
        else:
            tx = 0
        # width shift
        if self.width_range > 0:
            ty = random.uniform(-self.width_range, self.width_range) * x[0][1]
        else:
            ty = 0
            
        if self.depth_range > 0:
            tz = random.uniform(-self.depth_range, self.depth_range) * x[0][2]
        else:
            tz = 0
